var views = {
	appendControlPanel: function(){
		...
		divMain.innerHTML =
			'<div class="widget-title">' +
				...
				'<span id="' + COMMAND_PANEL_CLOSE_BUTTON_DOM_ID + '" class="btn btn-danger" style="float: right" title="Close the menu\nFeche o menu"><i class="fa fa-times" aria-hidden="true"></i></span>' +
				'<span id="' + INFO_ALERT + '" class="btn btn-warning" style="float: right" title="Information\nInformação"><i class="fa fa-info-circle" aria-hidden="true"></i></span>' +
				'<span id="' + CREDITS_INFO + '" class="btn btn-info" style="float: right" title="Credits\nCréditos"><i class="fa fa-users" aria-hidden="true"></i></span>' +
				'<span id="' + SET_CHAT_PANEL + '" class="btn btn-success" style="float: right" title="Chat room\nSala de bate-papo"><i class="fa fa-globe" aria-hidden="true"></i></span>' +
			'</div>' +
			'<div id="' + MAIN_SCREEN_DOM_ID + '" class="modal-body" style="max-height:405px">' +
				...
				'<tbody>' +
				'<tr class="info"><td>' + 
				'<button id="' + PERFORM_DELETE_SOFTWARE_ID + '" class="btn btn-success" title="Perform very easy missions\nExecutar missões muito fáceis">' + LANG.PERFORM_DELETE_SOFTWARE + '</button> '+
				'<button id="' + PERFORM_CHECK_BALANCE_ID + '" class="btn btn-warning" title="Perform bank check missions\nExecutar missões de verificação bancária">' + LANG.PERFORM_MEDIUM_MISSIONS + '</button> ' +
				'<button id="' + PERFORM_TRANSFER_MONEY_ID + '" class="btn btn-danger" title="Perform money transfer missions\nExecutar missões de transferência de dinheiro">' + LANG.PERFORM_HARD_MISSIONS + '</button>'+
				'<label><input class="checkBoxes" type="checkbox" id=' + SET_MISSIONS_MONITOR + '><span>' + LANG.NOTIFY_MISSIONS + '</span></label>' +
				'<button id="' + PERFORM_BANK_CAMPING + '" class="btn btn-success" title="Camp bank server and intercept transfers\nServidor de banco de camp e transferências de interceptação">' + LANG.INTERCEPT_TRANSACTIONS + '</button>' +
				selectIpList +
				...
				'<tr class="warning"><td>' + 
				'<button id="' + SET_CLEAN_OWN_LOGS_DOM_ID + '" class="btn btn-success" title="Clear local logs\nLimpar os tora locais">' + LANG.CLEAN_OWN_LOGS + '</button> ' +
				'<button id="' + SET_CLEAN_TARGET_LOGS_DOM_ID + '" class="btn btn-success" title="Clear remote logs\nLimpar tora remotos">' + LANG.CLEAN_VICTIM_LOGS + '</button> ' +
				'<button id="' + SET_ACCESS_TARGET_CLEAN_LOGS_DOM_ID + '" class="btn btn-success" title="Access remote server and clear logs\nAceder ao servidor remoto e limpar tora">' + LANG.ACCESS_CLEAR + '</button> ' +
				'<label><input class="checkBoxes" type="checkbox" id=' + SET_LOGS_MONITOR + '><span>' + LANG.NOTIFY_LOGS + '</span></label>' +
				'</td></tr>' +

				'<tr class="info"><td>' + 
				'<button id="' + SET_SEARCH_FOR_IPS + '" class="btn btn-success" title="Gather information about remote servers\nReunir informações sobre servidores remotos">' + LANG.RUN_WEBCRAWLER + '</button> ' + LANG.WEBCRAWLER_INITIAL_HOSTS + '<input id="' + FIELD_IPS_START_SEARCHING + '" class="controls fieldsContent" placeholder="' + LANG.PASTE_IPS + '" type="text" style="vertical-align: initial; margin-left: 10px; margin-right: 10px; width: 25%">' + 
			...
			'<div id="' + CREDITS_SCREEN_DOM_ID + '" class="modal-body">' +
				...
				LANG.CREDITS_OTHERS +
				'</div><br />' +
				'<button id="back-to-main" class="btn btn-success" title="Back to menu\nVoltar ao menu">' + LANG.CREDITS_BACK_BUTTON + '</button>'+
			'</div>' + 
			'<div id="' + CHAT_PANEL + '" class="modal-body" style="max-height:405px">' +
				'<h1 style="text-align:center;">HexBot CHAT</h1>' +
				...
				'<div class="btn-group" style="float: right">'+
					'<button type="button" style="float: right;" class="btn btn-info" id="' + CHAT_SEND_BUTTON + '" title="Click to send your message\nClique para enviar sua mensagem">' + LANG.CHAT_SEND + '</button>&nbsp&nbsp' +
					'<button type="button" style="float: right" class="btn btn-default" id="' + CHAT_REFRESH_BUTTON + '" title="Refresh to see new messages\nAtualizar para ver novas mensagens">' + LANG.CHAT_REFRESH + '</button> <br />' +
				'</div>'+
				
				'<button id="' + CHAT_TO_MAIN + '" class="btn btn-success" title="Back to menu\nVoltar ao menu">' + LANG.CREDITS_BACK_BUTTON + '</button>'+
			'</div>' +
	...
	showBotButton: function(){
		...
		aux.innerHTML = '<a><span id="botButtonContent" class="text" style="color: white;" title="Open the HexBot menu\nAbrir o menu HexBot"><i class="fa fa-cogs" aria-hidden="true"></i> <b>BOT</b></span></a>';
